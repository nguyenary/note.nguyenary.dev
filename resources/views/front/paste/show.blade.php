	...
	@if(config('settings.syntax_highlighter') == 'ace')
	  ...
	  @elseif(config('settings.syntax_highlighter') == 'codemirror')
	    ...
	    <style>.CodeMirror{ border:1px solid lightgray;max-height:{{ config('settings.paste_editor_height') * 20 . 'px' }}; }
	    	...
	    	.cm-url { color: #007bff; cursor: pointer; } 
		</style>
	  @else
		<!--PrismJs-->
...
$(document).on('click', '.cm-url', function(event) {
	...
	window.open('{{ route("redirect") }}?url='+link, '_blank'); 	
});
</script>
						...
						<div class="card-body">
							<div class="media"> @if(isset($paste->user))
								...
								<div class="media-body">
									...
									<p class="text-muted small"><i class="fa fa-user text-primary"></i> @if(isset($paste->user))
											<a href="{{$paste->user->url}}"> {{$paste->user->name}} </a> @else {{ __('Guest')}}  @endif
										<i class="fa fa-eye ml-2 text-secondary" title="{{ __('Views') }}"></i> {{$paste->views_f}}
										@if(!empty($paste->expire_time))<i class="fa fa-clock-o text-warning ml-2" title="{{ __('Expire') }}"></i> {{ $paste->expire_time_f }}@endif
										<i class="fa fa-calendar ml-2 text-success" title="{{ __('Pasted at') }}"></i> {{$paste->created_at->format('jS M, Y')}} @if($paste->status == 2)
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-body">
					<textarea class="form-control" id="embed_code"><iframe src="{{url('embed/'.$paste->slug)}}" style="border:none;width:100%;min-height:400px;"></iframe></textarea> 
