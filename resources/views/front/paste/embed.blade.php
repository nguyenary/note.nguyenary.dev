...
<div class="card-header"> {{$paste->title_f}} - <span class="badge badge-light">@if(!empty($paste->language)){{$paste->language->name}}@else{{strtoupper($paste->syntax)}}@endif</span> <small class="text-muted">{{$paste->content_size}} {{ __('KB') }}</small> 
